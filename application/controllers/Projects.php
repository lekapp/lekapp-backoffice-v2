...
class Projects extends CI_Controller {
	...
	public function edit( $speciality_role_id = 0 ){
		...
		$speciality = $this->speciality->obtener_ordenado();

		$speciality_array = array();

		foreach($speciality as $k => $v){
			$speciality_array[$k] = $v->name;
		}
		...
		if( $this->input->post('update') ){
			...
			if ($this->form_validation->run() == FALSE){
				...
				$this->load->view( SPATH . 'speciality_role_edit_structure', array( 'user' => $user[0], 'data' => $data[0], 'specialities' => $speciality_array ) );
				$this->load->view( CPATH . 'foot' );
			}
			else{
	...
	public function add(){
		...
		if( $this->input->post('add') ){
			...
			if ($this->form_validation->run() == FALSE){
				...
				$this->load->view( SPATH . 'speciality_role_add_structure', array( 'user' => $user[0], 'specialities' => $speciality_array ) );
				$this->load->view( CPATH . 'foot' );
			}
			else{
	...
	public function remove_activity( $activity_id = 0 ){

		$logged_in = $this->session->userdata('logged_in');

		if( $logged_in == FALSE || $activity_id == 0 ){
			redirect('login');
		}
		...
		$this->activity->borrar( $activity_id );
